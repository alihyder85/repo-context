"""Symbol and reference extractor for JavaScript source files."""
...
def _jsdoc(node: Any, source_bytes: bytes) -> Optional[str]:
    """Extract JSDoc comment preceding a node, if any (max 500 chars)."""
...
class JavaScriptExtractor(BaseExtractor):
    """Stateless extractor for JavaScript AST nodes."""

    def extract(self, parsed_file: ParsedFile) -> ExtractionResult:
        """Extract symbols and references from a parsed JavaScript file.
        ...
        ctx = ExtractionContext(file_path=file_path, language="javascript")
        ...
        try:
            ...
            logger.warning("JavaScriptExtractor: error in {}: {}", file_path, exc)
            elapsed = (time.perf_counter() - start) * 1000
            return ExtractionResult(
                file_meta=parsed_file.file_meta,
                symbols=ctx.symbols,
            ...
            for child in heritage.children:
                if child.type == "identifier":
                    ctx.references.append(Reference(
                        ...
                        callee_name=_node_text(child, src),
                        ref_type="inherit",
            ...
            if child.type == "import_clause":
                for sub in child.children:
                    if sub.type == "identifier":
                        imported = _node_text(sub, src)
                        ctx.references.append(Reference(
                            file_path=ctx.file_path,
                            caller_qualified_name=ctx.current_scope,
                            callee_name=f"{module}.{imported}",
                            ref_type="import",
                            line=node.start_point[0] + 1,
                        ))
